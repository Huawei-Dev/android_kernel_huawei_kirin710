	...
	int req)
{
	...
	hwlog_info("%s: req:%s, adc_supply_en_vote_cnt:%d\n",
		...
		mbhc_data->adc_supply_en_vote_cnt);

	if (req == ADC_POWER_SUPPLY_ENABLE) {
		mbhc_data->adc_supply_en_vote_cnt++;
		if (mbhc_data->adc_supply_en_vote_cnt == 1) {
			hwlog_info("%s: enable adc_power_supply", __func__);
			gpio_set_value(mbhc_data->adc_power_supply_en_gpio, 1);
		}
	} else {
		mbhc_data->adc_supply_en_vote_cnt--;
		if (mbhc_data->adc_supply_en_vote_cnt < 0) {
			mbhc_data->adc_supply_en_vote_cnt = 0;
		} else if (mbhc_data->adc_supply_en_vote_cnt == 0) {
	...
	int req)
{
	...
	hwlog_info("%s: req:%s, micbias_vote_cnt:%d\n",
		...
		"micbias disable", mbhc_data->micbias_vote_cnt);
	if (req == MICB_ENABLE) {
		mbhc_data->micbias_vote_cnt++;
		if (mbhc_data->micbias_vote_cnt == 1) {
		...
		mbhc_data->micbias_vote_cnt--;
		if (mbhc_data->micbias_vote_cnt < 0) {
			mbhc_data->micbias_vote_cnt = 0;
		} else if (mbhc_data->micbias_vote_cnt == 0) {
...
void hwevext_mbhc_plug_out_detect(struct hwevext_mbhc_priv *mbhc_data)
{
	IN_FUNCTION;
	mutex_lock(&mbhc_data->plug_mutex);
	mutex_lock(&mbhc_data->status_mutex);
...
static void hwevext_mbhc_repeat_detect_plug_workqueue(
	struct hwevext_mbhc_priv *mbhc_data)
{
...
{
	hwevext_mbhc_repeat_detect_plug_workqueue(mbhc_data);
	hwevext_mbhc_remove_read_hs_status_workqueue(mbhc_data);
	hwevext_mbhc_remove_btn_workqueue(mbhc_data);
	hwevext_mbhc_remove_plug_workqueue(mbhc_data);
	hwevext_mbhc_remove_long_press_btn_workqueue(mbhc_data);
