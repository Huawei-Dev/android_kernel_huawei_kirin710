            ...
            if (!use_sec_isp()) {
                if(!strncmp(entry->priv, "ISP_SHARED_MEM", strlen(entry->priv))) {
                    ...
                    pr_info("reserved_mems : %s, isp_share_para: %pK\n",
                        (char *)entry->priv, isp_share_para);
                }
            }
...
struct hisi_isp_ion_s *get_nesc_addr_ion(size_t size, size_t align, unsigned int heap_id_mask, unsigned int flags)
{
    ...
    hisi_nescaddr_ion = kzalloc(sizeof(struct hisi_isp_ion_s), GFP_KERNEL);
    ...
    ret = hisp_ion_phys(hisi_nescaddr_ion->ion_client, hisi_nescaddr_ion->ion_handle,(dma_addr_t *)&nesc_addr_ion_phys);
    if (ret < 0)
    {
											...
											int offset, int avail)
{
    ...
    else if (!strncmp(rsc->name, "ISP_MDH_MEM", strlen(rsc->name))){
        addr = get_mdc_addr_pa();
        if (!addr)
        {
...
int set_plat_parameters(void)
{
    ...
    param->rdr_enable_type         |= ISPCPU_RDR_USE_APCTRL;
...
static void rproc_set_shared_clk_init(void)
{
	...
	memset(share_para->clk_value, 0, sizeof(share_para->clk_value));
	hisp_unlock_sharedbuf();
...
static int hisi_rproc_start(struct rproc *rproc)
{
    ...
    spin_unlock_bh(&rproc_dev->rpmsg_ready_spin_mutex);
    return 0;
}
...
#define BOARDID_SIZE (4)
    ...
    switch(isplogic_type){
        case ISP_FPGA_EXCLUDE:
            ...
            break;
        case ISP_FPGA:
            if (dev->reg[PCTRL] == NULL) {
                RPROC_ERR("Failed : pctrl_base ioremap.\n");
                break;
            }
            pctrl_stat10 = readl((volatile void __iomem*)(dev->reg[PCTRL] + ISP_PCTRL_PERI_STAT_ADDR));
...
void coresight_mem_init(struct device *dev)
{
    ...
    g_cs_para.coresight_addr_da = (unsigned long long)dma_addr;
    g_cs_para.coresight_addr_vir = (unsigned long long)addr_vir;
...
static int hisp_regaddr_init(struct platform_device *pdev)
{
    ...
    for (i = 0; i < HISP_MIN(HISP_MAX_REGTYPE, rproc_dev->reg_num); i ++) {
...
static int hisi_rproc_probe(struct platform_device *pdev)
{
	...
	if ((ret = of_property_read_u32(np, "useloadbin", &rproc_dev->loadbin)) < 0 ) {
