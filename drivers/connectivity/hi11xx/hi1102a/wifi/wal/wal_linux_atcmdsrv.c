    ...
    ***************************************************************************/
    ...
    if (i_leftime == 0) {
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                         ...
                         ((WAL_ATCMDSRB_DBB_NUM_TIME * 1000) / OAL_TIME_HZ)); /* 1s????1000ms */
        return -OAL_EINVAL;
    } else if (i_leftime < 0) {
        /* ?????????????? */
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
...
OAL_STATIC oal_int32 wal_atcmsrv_ioctl_set_datarate(oal_net_device_stru *pst_net_dev, oal_int32 l_datarate)
{
    ...
    if (g_l_mode == WLAN_HT_ONLY_MODE) { /* ????????????7??????MCS7 */
...
OAL_STATIC oal_void wal_atcmdsrv_ioctl_convert_dbb_num(oal_uint32 ul_dbb_num, oal_uint8 *pc_dbb_num)
{
    ...
    uc_temp = (ul_dbb_num & 0xF0000000) >> 28; /* pc_dbb_num????BN????BN??1??????28-31bit?? */
    pc_dbb_num[0] = wal_atcmdsrv_get_hex_char(uc_temp);
    uc_temp = (ul_dbb_num & 0x0F000000) >> 24; /* pc_dbb_num????BN????BN??2??????24-27bit?? */
    pc_dbb_num[1] = wal_atcmdsrv_get_hex_char(uc_temp);

    pc_dbb_num[2] = '.'; /* pc_dbb_num[2]????'.' */

    uc_temp = (ul_dbb_num & 0x00F00000) >> 20; /* pc_dbb_num????Y1??????Y1??1??????20-23bit?? */
    pc_dbb_num[3] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[3]??Y1???????????????? */
    uc_temp = (ul_dbb_num & 0x000F0000) >> 16; /* pc_dbb_num????Y1??????Y1??2??????16-19bit?? */
    pc_dbb_num[4] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[4]??Y1???????????????? */
    pc_dbb_num[5] = '.'; /* pc_dbb_num[5]????'.' */

    uc_temp = (ul_dbb_num & 0x0000F000) >> 12; /* pc_dbb_num????Y2??????Y2????12-15bit?? */
    pc_dbb_num[6] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[6]??Y2???? */
    pc_dbb_num[7] = '.'; /* pc_dbb_num[7]????'.' */

    uc_temp = (ul_dbb_num & 0x00000F00) >> 8; /* pc_dbb_num????Y3??????Y2????8-11bit?? */
    pc_dbb_num[8] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[8]??Y1???????????????? */
    uc_temp = (ul_dbb_num & 0x000000F0) >> 4; /* pc_dbb_num????Y3??????Y2????4-7bit?? */
    pc_dbb_num[9] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[9]??Y1???????????????? */
    uc_temp = (ul_dbb_num & 0x0000000F) >> 0; /* pc_dbb_num????Y3??????Y2????0-3bit?? */
    pc_dbb_num[10] = wal_atcmdsrv_get_hex_char(uc_temp); /* pc_dbb_num[10]??Y1???????????????? */
...
OAL_STATIC oal_int32 wal_atcmsrv_ioctl_dieid_inform(oal_net_device_stru *pst_net_dev, oal_uint16 *pl_die_id)
{
    ...
    /* ????efuse???? */
    for (ul_loop = 0; ul_loop < 16; ul_loop++) { /* efuse buffer????????16 */
...
{
    if (!l_selfcali_en) {
        g_wlan_cal_disable_1102a = OAL_TRUE; /* ???????????????????? */
    } else {
        g_wlan_cal_disable_1102a = OAL_FALSE; /* ???????????????????? */
    }
        ...
        if (pst_scanned_bss_info == OAL_PTR_NULL) {
            oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_atcmsrv_ioctl_get_wifi_connect_info::find bss \
                fail by bssid:%02X:XX:XX:%02X:%02X:%02X}", pst_connect_info->auc_bssid[0],
                             ...
                             pst_connect_info->auc_bssid[5]); /* auc_bssid??5byte???????????????? */
    ...
    switch (st_priv_cmd.ul_cmd) {
        ...
        case WAL_ATCMDSRV_IOCTL_CMD_GET_CONN_INFO:
            ...
            OAL_IO_PRINT("wal_atcmsrv_ioctl_get_wifi_connect_info::status %d, %.32s,%02x:%02x:xx:xx:%02x:%02x,%d",
                         ...
                         st_priv_cmd.pri_data.st_connect_info.auc_bssid[4], /* auc_bssid??4byte???????????????? */
                         st_priv_cmd.pri_data.st_connect_info.auc_bssid[5], /* auc_bssid??5byte???????????????? */
