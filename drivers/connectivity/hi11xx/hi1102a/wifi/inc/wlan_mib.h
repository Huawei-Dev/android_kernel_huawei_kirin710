...
typedef enum {
    ...
    *************************************************************************/
    ...
    WLAN_CFGID_OPMODE_SWITCH = 224,            /* ??????????????rssi?????????????? */
    ...
    WLAN_CFGID_ANT_TAS_SWITCH_RSSI_NOTIFY = 681, /* TAS/??????????????RSSI????host */
#endif
    WLAN_CFGID_TAS_RSSI_ACCESS = 682,      /* TAS???????? */
    WLAN_CFGID_SET_EXT_FCC_CE_POWER = 683, /* ????FCC??CE?????????????????????????? */
    WLAN_CFGID_RX_FILTER_FRAG = 684, /* ???????????????? */
...
#endif
    WLAN_CFGID_GET_MNGPKT_SENDSTAT = 2034, /* ?????????????????? */
