...
/* ????????5??????11ac Operating Mode Notification??????????bit62??????????8,ftm??????????????9 */
...
OAL_STATIC OAL_INLINE oal_uint16  mac_get_seq_num(oal_uint8 *puc_header)
{
    ...
    us_seq_num <<= 4; /* puc_header[23]????4????puc_header[22]??4??????????????seqence number */
...
{
    puc_header[23]      = (oal_uint8)us_seq_num >> 4; /* puc_header[23]????us_seq_num????4?? */
    puc_header[22]      &= 0x0F; /* ????puc_header[22]????4?? */
    puc_header[22]      |= (oal_uint8)(us_seq_num << 4); /* us_seq_nume????4????????????puc_header[22]??4????????8bit?? */
}


OAL_STATIC OAL_INLINE oal_uint16 mac_get_bar_start_seq_num(oal_uint8 *puc_payload)
{
    /* puc_payload[2]??4????????????puc_payload[3]????4??????????????????????8????start seq num?? */
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 mac_is_addba_req_frame(oal_netbuf_stru *pst_netbuf, oal_uint8 *puc_tid)
{
    ...
    if (mac_get_frame_sub_type(puc_mac_header) == WLAN_FC0_SUBTYPE_ACTION) {
        if ((puc_mac_payload[0] == MAC_ACTION_CATEGORY_BA) && (puc_mac_payload[1] == MAC_BA_ACTION_ADDBA_REQ)) {
            *puc_tid     = (puc_mac_payload[3] & 0x3C) >> 2; /* ????puc_mac_payload[3]??2-5??????4??????tid */
            return OAL_TRUE;
        }
    }
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 mac_is_delba_frame(oal_netbuf_stru *pst_netbuf, oal_uint8 *puc_tid)
{
    ...
    if (mac_get_frame_sub_type(puc_mac_header) == WLAN_FC0_SUBTYPE_ACTION) {
        if ((puc_mac_payload[0] == MAC_ACTION_CATEGORY_BA) && (puc_mac_payload[1] == MAC_BA_ACTION_DELBA)) {
            *puc_tid     = (puc_mac_payload[3] & 0xF0) >> 4; /* ????puc_mac_payload[3]??4-7??????4??????tid */
            return OAL_TRUE;
        }
    }
...
{
    return ((puc_mac_header[0] & 0xF0) >> 4) ; /* puc_mac_header[0]????4??????4??????????????802.11????????????(0~15) */
}
...
{
    return (puc_mac_header[0] & 0x0C) >> 2; /* ????80211??????????????0~2 */
}
...
OAL_STATIC OAL_INLINE oal_uint16 mac_get_cap_info(oal_uint8 *mac_frame_body)
{
    ...
    /* mac_frame_body[us_index]????8????mac_frame_body[us_index + 1]????8??????8????????16????capability fied */
                                            ...
                                            oal_uint8         uc_offset)
{
    ...
    if (oal_unlikely((uc_use_btep1 == us_ether_type) || (uc_use_btep2 == us_ether_type))) {
        ...
        pst_llc->auc_org_code[2] = SNAP_BTEP_ORGCODE_2; /* auc_org_code??2byte 0xf8 */
    } else {
        pst_llc->auc_org_code[0]  = SNAP_RFC1042_ORGCODE_0;  /* 0x0 */
        pst_llc->auc_org_code[1]  = SNAP_RFC1042_ORGCODE_1;  /* 0x0 */
        pst_llc->auc_org_code[2]  = SNAP_RFC1042_ORGCODE_2;  /* auc_org_code??2byte 0x0 */
    }
...
OAL_STATIC OAL_INLINE oal_uint16  mac_get_auth_alg(oal_uint8 *puc_mac_hdr)
{
    ...
    /* puc_mac_hdr[MAC_80211_FRAME_LEN + 1]??8????puc_mac_hdr[MAC_80211_FRAME_LEN + 0]??8????????16?????????????? */
...
OAL_STATIC OAL_INLINE oal_uint16  mac_get_auth_status(oal_uint8 *puc_mac_hdr)
{
    ...
    us_auth_status = puc_mac_hdr[MAC_80211_FRAME_LEN + 5]; /* puc_mac_hdr[MAC_80211_FRAME_LEN]????5byte????auth_status */
    /* puc_mac_hdr[MAC_80211_FRAME_LEN + 5]??8????puc_mac_hdr[MAC_80211_FRAME_LEN + 4]??8????????16?????????????? */
...
OAL_STATIC OAL_INLINE oal_uint16  mac_get_auth_seq_num(oal_uint8 *puc_mac_hdr)
{
    ...
    us_auth_seq = puc_mac_hdr[MAC_80211_FRAME_LEN + 3]; /* puc_mac_hdr[MAC_80211_FRAME_LEN]????3byte????auth_seq */
    /* puc_mac_hdr[MAC_80211_FRAME_LEN + 3]??8????puc_mac_hdr[MAC_80211_FRAME_LEN + 2]??8????????16?????????????? */
...
{
    puc_hdr[1] &= 0xBF;
    puc_hdr[1] |= (oal_uint8)(uc_wep << 6); /* ??????????????????puc_hdr[1]????6???? */
}
...
{
    return ((puc_mac_hdr[1] & 0x40) >> 6); /* ????????????????????????????puc_hdr[1]????6???? */
}
...
OAL_STATIC OAL_INLINE oal_uint16  mac_get_auth_algo_num(oal_netbuf_stru *pst_netbuf)
{
    ...
    /* puc_mac_payload[1]????8????puc_mac_payload[0]????8????????16???????????????? */
...
{
    return &(puc_mac_hdr[MAC_80211_FRAME_LEN + 6]); /* ????????????challenge txt(puc_mac_hdr[MAC_80211_FRAME_LEN + 6]) */
}
...
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 4, WLAN_MAC_ADDR_LEN); /* ????????1(????head 4????) */
}


OAL_STATIC OAL_INLINE oal_void mac_get_address2  (oal_uint8 *puc_mac_hdr, oal_uint8 *puc_addr)
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 10, WLAN_MAC_ADDR_LEN); /* ????????2(????head??????1 10????) */
}

OAL_STATIC OAL_INLINE oal_void  mac_get_address3(oal_uint8 *puc_mac_hdr, oal_uint8 *puc_addr)
{
    memcpy_s(puc_addr, WLAN_MAC_ADDR_LEN, puc_mac_hdr + 16, WLAN_MAC_ADDR_LEN); /* ????????3(????head??????1??2 16????) */
}
...
OAL_STATIC OAL_INLINE mac_status_code_enum_uint16  mac_get_asoc_status(oal_uint8 *puc_mac_payload)
{
    ...
    /* puc_mac_payload[3]??8????puc_mac_payload[2]??8????????16???????????????????? */
...
OAL_STATIC OAL_INLINE oal_uint16 mac_get_asoc_id(oal_uint8 *puc_mac_payload)
{
    ...
    /* puc_mac_payload[5]??8????puc_mac_payload[4]??8????????16????????????????ID */
