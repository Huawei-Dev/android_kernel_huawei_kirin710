...
#define GET_HAL_DYN_CALI_DISABLE(_pst_handler) ((_pst_handler)->pst_hal_dev_rom->st_hal_dmac_dyn_cali.st_dyn_cali_val.aus_cali_en_interval[(_pst_handler)->st_wifi_channel_status.uc_band] == 0) /* ?????????????????????? */
#endif
#define GET_HAL_DEVICE_WORK_STATUS(_pst_handler) ((_pst_handler)->pst_hal_dev_rom->st_hal_dev_cb.p_mac_get_work_status(_pst_handler) == OAL_TRUE) /* ???????????????? */
#define GET_HAL_DEVICE_SCAN_STATUS(_pst_handler) ((_pst_handler)->pst_hal_dev_rom->st_hal_dev_cb.p_mac_get_scan_status(_pst_handler) == OAL_TRUE) /* ???????????????? */
