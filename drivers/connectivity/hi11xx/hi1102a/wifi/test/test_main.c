...
oal_uint32 wal_test_hipriv_reg_write(oal_int8 *pc_param)
{
    ...
    ul_addr = (oal_uint32)oal_strtol(pc_token, &pc_end, 16); /* 16???????????? */
    ...
    ul_val = (oal_uint32)oal_strtol(pc_token, &pc_end, 16); /* 16???????????? */
...
oal_void wal_long_rf_test(oal_void)
{
    ...
    for (ul_loop = 0; ul_loop < 50000; ul_loop++) { /* ??????????50000?? */
...
oal_uint32 wal_test_5115_reg_read(oal_int8 *pc_param)
{
    ...
    st_reg_info.ul_addr = (oal_uint32)oal_strtol(pc_token, &pc_end, 16); /* 16???????????? */
                                 ...
                                 oal_void *p_data)
{
    ...
    if (pc_cmd[0] == '1') {
        wal_test_hipriv_reg_write(&pc_cmd[2]); /* pc_cmd[2]??????????????test_hipriv_reg_write */
    } else if (pc_cmd[0] == '2') {
        wal_test_hipriv_reg_read(&pc_cmd[2]); /* pc_cmd[2]??????????????test_hipriv_reg_read */
    } else if (pc_cmd[0] == '3') {
        wal_long_rf_test();
    } else if (pc_cmd[0] == '4') {
        wal_long_mac_test();
    } else if (pc_cmd[0] == '5') {
        wal_test_5115_reg_read(&pc_cmd[2]); /* pc_cmd[2]??????????????test_5115_reg_read */
    } else {
