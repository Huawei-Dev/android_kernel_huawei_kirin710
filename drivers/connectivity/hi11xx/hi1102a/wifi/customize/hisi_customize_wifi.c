...
OAL_STATIC int32 g_host_init_params[WLAN_CFG_INIT_BUTT] = {0}; /* ini?????????????? */
int32 g_dts_params[WLAN_CFG_DTS_BUTT] = {0};        /* dts?????????????? */
OAL_STATIC uint8 g_nv_params[NUM_OF_NV_MAX_TXPOWER] = {0};    /* nv?????????????? */
int32 g_nvram_init_params[NVRAM_PARAMS_INDEX_BUTT] = {0}; /* ini??????NV???????? */
...
wlan_cus_pwr_fit_para_stru g_pro_line_params[DY_CALI_PARAMS_NUM] = {{0}};      /* ?????????????????? */
OAL_STATIC uint8 g_cust_nvram_info[WLAN_CFG_DTS_NVRAM_END][DY_CALI_PARAMS_LEN] = {{0}};  /* NVRAM???? */
oal_bool_enum_uint8 g_en_nv_dp_init_is_null = OAL_TRUE;                               /* NVRAM??dp init???????? */
wlan_customize_private_stru g_priv_cust_params[WLAN_CFG_PRIV_BUTT] = {{ 0, 0 }}; /* ?????????????????? */
oal_int16 g_gs_extre_point_vals[DY_CALI_NUM_5G_BAND] = {0};

wlan_customize_power_params_stru g_cust_nv_params = {{0}}; /* ?????????????????????? */
...
oal_void hwifi_set_nrcoex_rule(wlan_nrcoex_rule_stru *pst_nrcoex_rule, oal_uint8 uc_rule_index)
{
    ...
    /* ??????0?????????????? */
    /* ????????????????,bit31~bit24??160M,bit23~bit16??80M,bit15~bit8??40M,bit7~bit0??20M */
    ...
    /* ????RSSI?????? */
    /* ????????RSSI??????,bit31~bit24??160M,bit23~bit16??80M,bit15~bit8??40M,bit7~bit0??20M */
...
/*
 * ?? ?? ??  : hwifi_config_read_ext_fcc_ce_txpwr
 * ????????  : ????????????FCC??CE??????????????????????
 * ?? ?? ??  : OAL_SUCC ?? ??????????
 */
    ...
    if (en_nvm_initialed == OAL_FALSE) {
        if (hwifi_get_regdomain_from_country_code(hwifi_get_country_code()) != REGDOMAIN_FCC) {
            ...
            if ((l_ret == INI_SUCC) && (g_nv_params[0] != 0)) {
                en_nvm_initialed = OAL_TRUE; /* ????????????????????TRUE */
                return INI_SUCC;
            }
            oam_warning_log3 (0, OAM_SF_ANY,
                              ...
                              *((oal_uint32 *)(g_nv_params + sizeof(uint32)))); /* ??????8???????? */
        } else {
...
int8 *hwifi_get_country_code(void)
{
    ...
    if (ret == INI_FAILED) {
        ...
        /* ????????????????????????????????????99 */
        if (!OAL_MEMCMP(g_country_code, "99", sizeof("99") - 1)) { /* ?????????????? */
    ...
    g_country_code[COUNTRY_CODE_LEN - 1] = '\0'; /* ??'\0'???? */
