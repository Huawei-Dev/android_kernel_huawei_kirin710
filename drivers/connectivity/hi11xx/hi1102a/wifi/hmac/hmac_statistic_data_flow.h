...
typedef struct {
    oal_atomic ul_tx_bytes; /* WIFI ?????????????? */
    oal_atomic ul_rx_bytes; /* WIFI ?????????????? */
...
typedef struct {
    ...
    oal_uint8 uc_tx_opt_switch_ap;   /*
                                      ...
                                      * BIT5-BIT7: RESERVE
                                      */
    oal_uint8 uc_cwmin;              /* ????????????cwmin */
    oal_uint8 uc_cwmax;              /* ????????????cwmax */
    oal_uint8 uc_resv;
    oal_uint16 us_txoplimit;         /* ????????????txoplimit */
    oal_uint16 us_dyn_txoplimit;     /* ????????????????txoplimit */
