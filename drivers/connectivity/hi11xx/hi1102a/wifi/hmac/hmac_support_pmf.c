                                  ...
                                  oal_uint16 us_init_trans_id)
{
    ...
    pst_tx_ctl->us_tx_user_idx = pst_hmac_user->st_user_base_info.us_assoc_id;  /* ????????????????user?????? */
                              ...
                              oal_bool_enum_uint8 en_is_protected)
{
    ...
    /* ????trans_id */
    pus_trans_id = (oal_uint16 *)(puc_mac_hdr + MAC_80211_FRAME_LEN + 2); /* 2????duration */
    pst_sa_query_info = &pst_hmac_user->st_sa_query_info;

    /* ??????????SA query reqponse????????????????SA */
    if (oal_memcmp(pus_trans_id, &(pst_sa_query_info->us_sa_query_trans_id), 2) == 0) { /* 2?????????????????? */
        /* pending SA Query requests ?????????? & sa query???????????????? */
        pst_sa_query_info->ul_sa_query_count = 0;
        pst_sa_query_info->ul_sa_query_start_time = 0;
