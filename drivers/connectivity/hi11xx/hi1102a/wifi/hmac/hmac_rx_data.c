                               ...
                               mac_msdu_proc_status_enum_uint8 *pen_proc_state)
{
    ...
    oal_uint16 us_offset;                       /* submsdu??????data?????????? */
...
OAL_STATIC oal_void hmac_parse_ipv4_packet(oal_void *pst_eth, oal_uint32 netbuf_len)
{
    ...
    iphdr_len = iph->ihl * 4; /* iph->ihl????IP????????????????4???? */
...
OAL_STATIC oal_void hmac_parse_arp_packet(oal_void *pst_eth, oal_uint32 buf_len)
{
    ...
    iph = (struct iphdr *)((mac_ether_header_stru *)pst_eth + 1); /* ?????????????????? ????????6 ??????6 ??????2????ip?? */
    iphdr_len = iph->ihl * 4; /* ????ip??????????ihl * 4 */
    arp = (struct arphdr *)((oal_uint8 *)iph + iphdr_len); /* ??ip????????arp???????????? */
        ...
        if (OAL_HOST2NET_SHORT(ETHER_TYPE_IP) == pst_ether_hdr->us_ether_type) {
            if (mac_vap_find_user_by_macaddr(pst_mac_vap, pst_ether_hdr->auc_ether_shost, &us_assoc_id) != OAL_SUCC) {
                oam_warning_log4(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                                 ...
                                 (oal_uint32)(pst_ether_hdr->auc_ether_shost[3]), /* auc_ether_shost??3byte???????? */
                                 (oal_uint32)(pst_ether_hdr->auc_ether_shost[4]), /* auc_ether_shost??4byte???????? */
                                 (oal_uint32)(pst_ether_hdr->auc_ether_shost[5])); /* auc_ether_shost??5byte???????? */
                oal_netbuf_free(pst_netbuf);
                return;
            }
...
oal_uint32 hmac_rx_check_snap(oal_uint8 *puc_payload)
{
    ...
    if ((pst_snap->auc_org_code[0] != SNAP_RFC1042_ORGCODE_0) ||
        ...
        (pst_snap->auc_org_code[2] != SNAP_RFC1042_ORGCODE_2)) { /* ????auc_org_code0??1??2byte???????? */
        return OAL_FAIL;
    }
