...
oal_uint16 hmac_mgmt_encap_auth_req(hmac_vap_stru *pst_hsta, oal_uint8 *puc_mgmt_frame)
{
    ...
    if (mac_mib_get_privacyinvoked(&pst_hsta->st_vap_base_info) == OAL_FALSE) {
        ...
        puc_mgmt_frame[MAC_80211_FRAME_LEN + 1] = ((us_auth_type & 0xFF00) >> 8); /* ????us_auth_type????8??????????8?? */
    }

    /* ???? Authentication Transaction Sequence Number ?? 1 */
    /* Authentication Transaction Sequence Number??1??????MAC_80211_FRAME_LEN??????2byte??????1 */
    puc_mgmt_frame[MAC_80211_FRAME_LEN + 2] = 0x01;
    /* Authentication Transaction Sequence Number??2??????MAC_80211_FRAME_LEN??????3byte?????? */
    puc_mgmt_frame[MAC_80211_FRAME_LEN + 3] = 0x00;

    /* ???? Status Code ??0. ???????????????????? . */
    puc_mgmt_frame[MAC_80211_FRAME_LEN + 4] = 0x00; /* Status Code??1??????MAC_80211_FRAME_LEN??????4byte?????? */
    puc_mgmt_frame[MAC_80211_FRAME_LEN + 5] = 0x00; /* Status Code??2??????MAC_80211_FRAME_LEN??????5byte?????? */
...
oal_uint16 hmac_mgmt_encap_auth_req_seq3(hmac_vap_stru *pst_sta, oal_uint8 *puc_mgmt_frame, oal_uint8 *puc_mac_hrd)
{
    ...
    puc_data[2] = 0x03; /* Authentication Transaction Sequence Number??1????puc_data??2byte????????3 */
    puc_data[3] = 0x00; /* Authentication Transaction Sequence Number??2????puc_data??3byte?????? */
    ...
    if (mac_is_protectedframe(puc_mac_hrd) == 1) {
        puc_data[4] = MAC_CHLNG_FAIL; /* Status Code??1????puc_data??4byte????????????????????1,??????MAC_CHLNG_FAIL */
        puc_data[5] = 0x00; /* Status Code??2????puc_data??5byte?????? */
    ...
    /* status, since WEP is necessary for Shared Key Authentication. */
        puc_data[4] = MAC_UNSUPT_ALG; /* Status Code??1????puc_data??4byte??????????????????WEP????????MAC_UNSUPT_ALG */
        puc_data[5] = 0x00; /* Status Code??2????puc_data??5byte?????? */
    ...
    /* status, since a NULL key value cannot be used for WEP operations. */
        puc_data[4] = MAC_CHLNG_FAIL; /* Status Code??1????puc_data??4byte????????????WEP??????NULL, ??????MAC_CHLNG_FAIL */
        puc_data[5] = 0x00; /* Status Code??2????puc_data??5byte?????? */
    } else {
        ...
        puc_data[4] = MAC_SUCCESSFUL_STATUSCODE; /* Status Code??1????puc_data??4byte?????????????????????????????? */
        puc_data[5] = 0x00; /* Status Code??2????puc_data??5byte?????? */

        /* Extract 'Challenge Text' and its 'length' from the incoming */
        /* authentication frame */
        uc_ch_text_len = puc_mac_hrd[MAC_80211_FRAME_LEN + 7]; /* ????ch_text_len??MAC_80211_FRAME_LEN????7byte?? */
        /* ????ch_text??MAC_80211_FRAME_LEN????8byte?? */
        ...
        /* | 1         |1       |1 - 253         | */
        /* --------------------------------------- */
        puc_mgmt_frame[us_index + 6] = MAC_EID_CHALLENGE; /* MAC_EID_CHALLENGE??????us_index????6byte */
        puc_mgmt_frame[us_index + 7] = uc_ch_text_len; /* ch_text_len??????us_index????7byte */
