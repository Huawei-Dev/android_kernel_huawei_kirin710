...
#ifdef CONFIG_HAS_EARLYSUSPEND
    pst_hmac_device->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 20; /* 20????early_suspend?????? */
...
oal_void hmac_device_set_random_mac_for_scan(mac_device_stru *pst_mac_dev, mac_vap_stru *pst_mac_vap)
{
    ...
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                     ...
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[3], /* auc_random_mac??3byte?????????????? */
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[4], /* auc_random_mac??4byte?????????????? */
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[5]); /* auc_random_mac??5byte?????????????? */
