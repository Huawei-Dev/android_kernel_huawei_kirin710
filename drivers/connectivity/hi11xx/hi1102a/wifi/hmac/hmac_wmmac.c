                                                ...
                                                oal_uint16 us_frame_len)
{
    ...
    /* DA is ADDTS Request frame's SA??????4byte??????DA?????? */
                                            ...
                                            oal_uint8 uc_tsid)
{
    ...
    if (IS_STA(&(pst_vap->st_vap_base_info))) {
        ...
        /* DA is puc_addr, user's address??????4byte??????DA?????? */
                                  ...
                                  oal_uint32 frame_body_len)
{
    ...
    /* ----------------------------------------------------------------------- */
    /*************************************************************************/
    pst_tspec_info = (mac_wmm_tspec_stru *)(puc_payload + 12); /* 12??????????tspec???????? */
    ...
    if (uc_user_prio >= WLAN_TID_MAX_NUM) {
        ...
        oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC,
                         ...
                         uc_user_prio, puc_payload[2], puc_payload[3]); /* puc_payload??2??3?????????????????? */
        return OAL_SUCC;
    }
    ...
    /* ??????????????ts???? */
    pst_ts = &(pst_hmac_user->st_user_base_info.st_ts_info[uc_ac_num]);
    uc_dialog_token = puc_payload[2]; /* puc_payload??2??????Dialog token */
    ...
    /* ????ADDTS RSP??????????dmac TS???? */
    if (puc_payload[3] == MAC_SUCCESSFUL_STATUSCODE) { /* puc_payload??3byte??Status */
                              ...
                              oal_uint32 frame_body_len)
{
    ...
    if (uc_tsid >= WLAN_TID_MAX_NUM) {
        ...
        oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_mgmt_rx_delts::tsid[%d]} token[%d] status[%d]",
                         uc_tsid, puc_payload[2], puc_payload[3]); /* puc_payload??2??3?????????????????? */
        return OAL_SUCC;
    }
    /* ????????AC????????????TSID??????TS???? */
