                            ...
                            oal_uint8 *puc_key, oal_uint8 *puc_bufout)
{
    ...
    ul_counter = ul_buflen / 16; /* ul_buflen??16??????????counter */
    ul_comp = ul_buflen % 16; /* ul_buflen??16??????????comp */
    ...
    for (ul_loop = 0; ul_loop < ul_counter; ul_loop++) {
        ...
        pul_out[2] = pul_in[2] ^ aul_iv_out[2]; /* pul_in[2]??aul_iv_out[2]??????????????pul_out[2] */
        pul_out[3] = pul_in[3] ^ aul_iv_out[3]; /* pul_in[3]??aul_iv_out[3]??????????????pul_out[3] */
        hmac_sms4_crypt((oal_uint8 *)aul_iv_out, (oal_uint8 *)aul_iv_out, aul_pr_keyin, sizeof(aul_pr_keyin));
        pul_in += 4; /* pul_in????4byte */
        pul_out += 4; /* pul_out????4byte */
    }
...
void hmac_wpi_swap_pn(oal_uint8 *puc_pn, oal_uint8 uc_len)
{
    ...
    uc_len_tmp = uc_len / 2; /* ??PN??????????for????uc_len / 2???? */
                         ...
                         oal_uint8 *puc_key, oal_uint8 *puc_mic, oal_uint8 uc_mic_len)
{
    ...
    if ((ul_pamclen < 1) || (ul_pamclen > 4096)) { /* pamclen 1-4096???????????????? */
        return OAL_FAIL;
    }
    ...
    for (ul_loop = 0; ul_loop < ul_pamclen; ul_loop++) {
        ...
        aul_mic_tmp[2] ^= pul_in[2]; /* aul_mic_tmp[2]??pul_in[2]???????? */
        aul_mic_tmp[3] ^= pul_in[3]; /* aul_mic_tmp[3]??pul_in[3]???????? */
        pul_in += 4; /* pul_in????4byte */
    ...
    pul_in[2] = aul_mic_tmp[2]; /* aul_mic_tmp[2]??????pul_in[2] */
    pul_in[3] = aul_mic_tmp[3]; /* aul_mic_tmp[3]??????pul_in[3] */
