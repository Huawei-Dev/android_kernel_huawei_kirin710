...
oal_uint16 g_wlan_assoc_user_max_num = 32; /* ?????????????????? Root AP????????32??,Repeater????????15?? */
...
#define CHR_WEB_FAIL_ROAM_RX_RATE_TH 13500 /* CHR??????????????????????????rx rate???? */
#define CHR_WEB_FAIL_ROAM_RSSI_TH (-55) /* CHR??????????????????????????RSSI????,???????????????????????? */
#define CHR_WEB_FAIL_ROAM_MIN_INTERVAL 600000 /* CHR??????????????????, 10???? */
#define CHR_WEB_FAIL_ROAM_PROTECT_PERIOD 60000 /* ??????????????????????????,??????????????????1????,?????????????? */
/* 3 ????????  TBD ???????????? */
...
oal_void hmac_custom_init(oal_uint32 ul_psta_enable)
{
     ...
     */
    hmac_set_wlan_assoc_user_max_num(8); /* ????????????????????8?? */
...
oal_void hmac_chr_process_web_fail(oal_uint32 ul_chr_event_id, oal_uint32 ul_rx_rate)
{
    ...
    if (mac_device_calc_up_vap_num(pst_mac_device) != 1) { /* 1????????????VAP */
        return;
    }
