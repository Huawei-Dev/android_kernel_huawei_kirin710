...
OAL_STATIC oal_uint32 hmac_wifi_calculate_stat(wifi_txrx_calculated_stat_stru *pst_calculated_stat)
{
    ...
    pst_calculated_stat->ul_tx_large_pps = (oal_atomic_read(&pst_pkgs_info->ul_tx_large_pkt) * 1000) / /* 1s????1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_rx_large_pps = (oal_atomic_read(&pst_pkgs_info->ul_rx_large_pkt) * 1000) / /* 1s????1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_tx_small_pps = (oal_atomic_read(&pst_pkgs_info->ul_tx_small_pkt) * 1000) / /* 1s????1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_rx_small_pps = (oal_atomic_read(&pst_pkgs_info->ul_rx_small_pkt) * 1000) / /* 1s????1000ms */
                                                 ul_dur_ms;
    /* ????ul_rx_bytes??????????????????????7?? */
