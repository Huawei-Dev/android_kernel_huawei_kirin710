    ...
    if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (pst_hmac_user_btcoex->st_hmac_btcoex_addba_req.en_ba_handle_allow == OAL_FALSE) {
            if (hmac_btcoex_get_blacklist_type(pst_hmac_user) == BTCOEX_BLACKLIST_TPYE_FIX_BASIZE) {
                *pus_ba_param |= (oal_uint16)(0x0008 << 6); /* 0x0008??????6??????????pus_ba_param????9bit?? */
            } else {
                /* ??????????btcoex??????????????????????????????????????????wifi???? */
 ...
 */
    ...
    puc_data[us_index++] = (oal_uint8)((us_ba_param >> 8) & 0xFF); /* ????us_ba_param????8????B8-B15?? */

    /* ????BlockAck timeout */
    puc_data[us_index++] = (oal_uint8)(pst_tx_ba->us_ba_timeout & 0xFF);
    puc_data[us_index++] = (oal_uint8)((pst_tx_ba->us_ba_timeout >> 8) & 0xFF); /* ????us_ba_timeout????8????B8-B15?? */
                                     ...
                                     oal_uint8 uc_status)
{
    ...
    /* SA is the dot11MACAddress??????10byte??????SA?????? */
    oal_set_mac_addr(puc_data + 10, pst_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_sta_config.auc_dot11StationID);
    ...
    oam_warning_log2(0, OAM_SF_BA, "hmac_mgmt_encap_addba_rsp:tid[%d],ba_size[%d]", uc_tid,
                     ((us_ba_param & 0xffc0) >> 6)); /* ????ba_size????us_ba_param B6-B15?? */
    puc_data[us_index++] = (oal_uint8)(us_ba_param & 0xFF);
    puc_data[us_index++] = (oal_uint8)((us_ba_param >> 8) & 0xFF); /* ????us_ba_param????8????B8-B15?? */
                                 ...
                                 oal_uint8 reason)
{
    ...
    /* B12-B15 - TID */
    puc_data[us_index++] = 0;
    puc_data[us_index] = (oal_uint8)(en_initiator << 3); /* initiator????3?? */
    puc_data[us_index++] |= (oal_uint8)(uc_tid << 4); /* tid????4?? */
    ...
    hmac_user_stru *pst_hmac_user, dmac_ba_tx_stru *pst_tx_ba, mac_action_mgmt_args_stru *pst_action_args)
{
    ...
    pst_tx_ba->uc_dialog_token = pst_hmac_vap->uc_ba_dialog_token; /* ????ba???????????????? */
...
{
    /* puc_payload 7byte????4???? 8byte????4????????8bit??baw_start */
    pst_ba_rx_stru->us_baw_start = (puc_payload[7] >> 4) | (puc_payload[8] << 4);
    pst_ba_rx_stru->us_baw_size = (puc_payload[3] & 0xC0) >> 6; /* payload??3 byte??2??????4 byte????6????????baw_size */
...
{
    /* puc_payload 5byte????8????6byte????8??????16bit??ba_timeout */
    ...
    pst_ba_rx_stru->uc_ba_policy = (puc_payload[3] & 0x02) >> 1; /* puc_payload[3]????1bit??????_ba_policy */
}
                                  ...
                                  oal_uint32 frame_body_len)
{
    ...
    /* --------------------------------------------------------------- */
    /******************************************************************/
    uc_tid = (puc_payload[3] & 0x3C) >> 2; /* ????puc_payload[3]??B2-B5????tid */
    ...
    pst_ba_rx_stru->uc_dialog_token = puc_payload[2]; /* puc_payload??2byte??Dialog token */
    ...
    mac_vap_stru *pst_mac_vap, mac_action_mgmt_args_stru *pst_action_args, oal_uint8 uc_tidno)
{
    ...
    pst_action_args->ul_arg1 = uc_tidno;                                            /* ??????????????TID?? */
    pst_action_args->ul_arg2 = MAC_ORIGINATOR_DELBA;                               /* DELBA????????????BA???????????? */
    pst_action_args->ul_arg3 = MAC_UNSPEC_QOS_REASON;                               /* DELBA??????????reason */
    pst_action_args->puc_arg5 = pst_hmac_user->st_user_base_info.auc_user_mac_addr; /* DELBA?????????????? */
    ...
    dmac_ctx_action_event_stru *pst_rx_addba_rsp_event)
{
    ...
    /* ??????????????????????????????????????????dmac */
    pst_rx_addba_rsp_event->uc_ba_policy = ((puc_payload[5] & 0x02) >> 1); /* uc_ba_policy??puc_payload[5]??1bit */
    /* puc_payload 7byte????8????puc_payload 8byte????8??????16bit??ba_timeout */
    pst_rx_addba_rsp_event->us_ba_timeout = puc_payload[7] | (puc_payload[8] << 8);
    pst_rx_addba_rsp_event->en_amsdu_supp = puc_payload[5] & BIT0;  /* en_amsdu_supp??puc_payload[5]????0bit */
    /* puc_payload 5byte??????2????puc_payload 6byte????????6????????baw_size */
                                  ...
                                  oal_uint32 frame_body_len)
{
    ...
    /* --------------------------------------------------------------- */
    /******************************************************************/
    uc_tidno = (puc_payload[5] & 0x3C) >> 2; /* ????puc_payload[5]??2-5bit??????2????????tidno */
    /* ????????tid??0~15,02??????tid0~7 */
    if (uc_tidno >= WLAN_TID_MAX_NUM) {
        ...
        oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::tid[%d]token[%d]state[%d]",
                         uc_tidno, puc_payload[2], puc_payload[3]); /* puc_payload??2??3byte?????????????? */
        return OAL_SUCC;
    }

    uc_dialog_token = puc_payload[2]; /* puc_payload??2byte??Dialog token */
    uc_ba_status = puc_payload[3]; /* puc_payload??3byte??Status */
    uc_ba_policy = ((puc_payload[5] & 0x02) >> 1); /* ba_policy??puc_payload[5]??1bit */
    ...
    pst_rx_addba_rsp_event->uc_status = puc_payload[3]; /* puc_payload??3byte??Status */
    pst_rx_addba_rsp_event->uc_tidno = uc_tidno;
    pst_rx_addba_rsp_event->uc_dialog_token = puc_payload[2]; /* puc_payload??2byte?? Dialog token */
                              ...
                              oal_uint32 frame_body_len)
{
    ...
    /* -------------------------------------------- */
    /************************************************/
    uc_tid = (puc_payload[3] & 0xF0) >> 4; /* puc_payload 3byte????4????tid */
    uc_initiator = (puc_payload[3] & 0x08) >> 3; /* puc_payload 3byte????3bit??initiator */
    /* puc_payload 4byte????8????5byte????8??????8????????16????Reason */
                                     ...
                                     oal_bool_enum_uint8 en_is_protected)
{
    ...
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                     ...
                     puc_da[0], puc_da[4], puc_da[5], us_err_code); /* puc_da??0??4??5byte?????????????? */
                                       ...
                                       oal_bool_enum_uint8 en_is_protected)
{
    ...
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                     ...
                     us_err_code, puc_da[0], puc_da[4], puc_da[5]); /* puc_da??0??4??5byte?????????????? */
...
oal_void hmac_set_user_protocol_mode(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        ...
        if (pst_mac_vap->st_channel.en_band == WLAN_BAND_5G) { /* ??????????5G */
...
oal_uint32 hmac_proc_ht_cap_ie(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 *puc_ht_cap_ie)
{
    ...
    /* ???? HTC support Information??uc_htc_support??us_ht_cap_info????10bit???? */
    ...
    /* bit_transmit_staggered_sounding_cap????2bit */
    pst_ht_hdl->bit_transmit_staggered_sounding_cap = ((ul_tmp_txbf_elem & BIT2) >> 2);
    pst_ht_hdl->bit_receive_ndp_cap = ((ul_tmp_txbf_elem & BIT3) >> 3); /* bit_receive_ndp_cap????3bit */
    pst_ht_hdl->bit_transmit_ndp_cap = ((ul_tmp_txbf_elem & BIT4) >> 4); /* bit_transmit_ndp_cap????4bit */
    pst_ht_hdl->bit_imbf_cap = ((ul_tmp_txbf_elem & BIT5) >> 5); /* bit_imbf_cap????5bit */
    pst_ht_hdl->bit_calibration = ((ul_tmp_txbf_elem & 0x000000C0) >> 6); /* bit_calibration????6bit */
    pst_ht_hdl->bit_exp_csi_txbf_cap = ((ul_tmp_txbf_elem & BIT8) >> 8); /* bit_exp_csi_txbf_cap????8bit */
    pst_ht_hdl->bit_exp_noncomp_txbf_cap = ((ul_tmp_txbf_elem & BIT9) >> 9); /* bit_exp_noncomp_txbf_cap????9bit */
    pst_ht_hdl->bit_exp_comp_txbf_cap = ((ul_tmp_txbf_elem & BIT10) >> 10); /* bit_exp_comp_txbf_cap????10bit */
    pst_ht_hdl->bit_exp_csi_feedback = ((ul_tmp_txbf_elem & 0x00001800) >> 11); /* bit_exp_csi_feedback????11bit */
    /* bit_exp_noncomp_feedback????13bit */
    pst_ht_hdl->bit_exp_noncomp_feedback = ((ul_tmp_txbf_elem & 0x00006000) >> 13);
    pst_ht_hdl->bit_exp_comp_feedback = ((ul_tmp_txbf_elem & 0x0001C000) >> 15); /* bit_exp_comp_feedback????15bit */
    pst_ht_hdl->bit_min_grouping = ((ul_tmp_txbf_elem & 0x00060000) >> 17); /* bit_min_grouping????17bit */
    pst_ht_hdl->bit_csi_bfer_ant_number = ((ul_tmp_txbf_elem & 0x001C0000) >> 19); /* bit_csi_bfer_ant_number????19bit */
    /* bit_noncomp_bfer_ant_number????21bit */
    pst_ht_hdl->bit_noncomp_bfer_ant_number = ((ul_tmp_txbf_elem & 0x00600000) >> 21);
    /* bit_comp_bfer_ant_number????23bit */
    pst_ht_hdl->bit_comp_bfer_ant_number = ((ul_tmp_txbf_elem & 0x01C00000) >> 23);
    pst_ht_hdl->bit_csi_bfee_max_rows = ((ul_tmp_txbf_elem & 0x06000000) >> 25); /* bit_csi_bfee_max_rows????25bit */
    pst_ht_hdl->bit_channel_est_cap = ((ul_tmp_txbf_elem & 0x18000000) >> 27); /* bit_channel_est_cap????27bit */
    ...
    mac_vht_hdl_stru *pst_mac_vht_hdl, oal_uint32 ul_vht_cap_field)
{
    ...
    /* ????max_mpdu_length??????2??????????MAX_MPDU_LEN_HIGH_VHT_CAP??11454 */
                                ...
                                oal_uint8 *puc_vht_cap_ie)
{
    ...
    /* ????rx_ldpc */
    pst_mac_vht_hdl->bit_rx_ldpc = ((ul_vht_cap_field & BIT4) >> 4); /* bit_rx_ldpc????4bit */

    /* ????short_gi_80mhz??short_gi_160mhz???????? */
    pst_mac_vht_hdl->bit_short_gi_80mhz = ((ul_vht_cap_field & BIT5) >> 5); /* bit_short_gi_80mhz????5bit */
    pst_mac_vht_hdl->bit_short_gi_80mhz &=
        pst_mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTShortGIOptionIn80Implemented;

    pst_mac_vht_hdl->bit_short_gi_160mhz = ((ul_vht_cap_field & BIT6) >> 6); /* bit_short_gi_160mhz????6bit */
    ...
    /* ????tx_stbc ??rx_stbc */
    pst_mac_vht_hdl->bit_tx_stbc = ((ul_vht_cap_field & BIT7) >> 7); /* bit_tx_stbc????7bit */
    /* bit_rx_stbc??8bit??9bit??10bit?????? */
    pst_mac_vht_hdl->bit_rx_stbc = ((ul_vht_cap_field & (BIT10 | BIT9 | BIT8)) >> 8);

    /* ????su_beamformer_cap??su_beamformee_cap */
    pst_mac_vht_hdl->bit_su_beamformer_cap = ((ul_vht_cap_field & BIT11) >> 11); /* bit_su_beamformer_cap????11bit */
    pst_mac_vht_hdl->bit_su_beamformee_cap = ((ul_vht_cap_field & BIT12) >> 12); /* bit_su_beamformee_cap????12bit */

    /* ????num_bf_ant_supported??bit_num_bf_ant_supported??13bit??14bit??15bit???????? */
    ...
    /* ????num_sounding_dim??bit_num_sounding_dim??16bit??17bit??18bit???????? */
    pst_mac_vht_hdl->bit_num_sounding_dim = ((ul_vht_cap_field & (BIT18 | BIT17 | BIT16)) >> 16);

    /* ????mu_beamformer_cap??mu_beamformee_cap */
    pst_mac_vht_hdl->bit_mu_beamformer_cap = ((ul_vht_cap_field & BIT19) >> 19); /* bit_mu_beamformer_cap????19bit */
    pst_mac_vht_hdl->bit_mu_beamformee_cap = ((ul_vht_cap_field & BIT20) >> 20); /* bit_mu_beamformee_cap????20bit */

    /* ????vht_txop_ps??bit_vht_txop_ps????21bit?? */
