    ...
    oal_uint8 *puc_payload, oal_uint16 us_pdu_len, oal_uint8 *puc_mic, oal_uint16 us_mic_len)
{
    ...
    /* keyidx + reserve????2?????? */
    *puc_mic = uc_keyidx;
    puc_mic += 2; /* ????keyidx + reserve??2byte?? */

    /* ????pdulen ?????????????????? */
    *puc_mic = (oal_uint8)((us_pdu_len & 0xff00) >> 8); /* ????us_pdu_len??8-15??????8???? */
    *(puc_mic + 1) = (oal_uint8)(us_pdu_len & 0x00ff);
    puc_mic += 2; /* ????????pdulen ????????????????????2byte */
...
oal_uint32 hmac_wapi_decrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru *pst_netbuf)
{
    ...
    ul_wapi_result = hmac_wpi_pmac(puc_pn, puc_mic_data, (us_mic_len >> 4), puc_key_ck, auc_calc_mic, /* mic_len????4?? */
    ...
    if (oal_memcmp(puc_mic, auc_calc_mic, SMS4_MIC_LEN) != 0) {  /* ????MIC */
...
oal_void hmac_wapi_dump_frame(oal_uint8 *puc_info, oal_uint8 *puc_data, oal_uint32 ul_len)
{
    oal_uint32 ul_loop;
    OAL_IO_PRINT("%s: \r\n", puc_info);
    for (ul_loop = 0; ul_loop < ul_len; ul_loop += 4) { /* ??4??byte???? */
        OAL_IO_PRINT("%2x ", ul_loop / 4); /* ??4??byte???? */
        OAL_IO_PRINT("%2x %2x %2x %2x \r\n", puc_data[ul_loop], puc_data[ul_loop + 1],
                     puc_data[ul_loop + 2], puc_data[ul_loop + 3]); /* puc_data[ul_loop]????2??3byte?????????????? */
    }
}
