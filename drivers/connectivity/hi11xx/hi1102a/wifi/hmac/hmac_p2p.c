...
oal_uint32 hmac_find_p2p_listen_channel(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* ????P2P Listen channel???? */
    /* puc_p2p_ie????6byte??MAC_WLAN_OUI_WFA(3byte)??MAC_EID_VENDOR(1byte)??lenth(1byte)??id(1byte)????p2p ie??1byte??4 */
    ...
    /* listen channel????????????????????listen channel??????????puc_listen_channel_ie??2byte????8????????1byte?????? */
    if (MAC_P2P_LISTEN_CHN_ATTR_LEN != (oal_int32)((puc_listen_channel_ie[2] << 8) + puc_listen_channel_ie[1])) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         ...
                         /* puc_listen_channel_ie[2]????8????puc_listen_channel_ie[1]???????????????? */
                         (oal_int32)((puc_listen_channel_ie[2] << 8) + puc_listen_channel_ie[1]));
        return OAL_FAIL;
    }

    /* ????P2P Listen channel????(puc_listen_channel_ie??7byte) */
