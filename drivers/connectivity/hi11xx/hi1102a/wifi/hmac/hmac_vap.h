...
#endif
#define HMAC_STA_RSSI_LOG_NUM 3 /* STA????RSSI?????????? */
/*****************************************************************************
  3 ????????
*****************************************************************************/
/*****************************************************************************
...
typedef struct {
    ...
    oal_bool_enum_uint8              uc_efuse_reg_flag;              /* efuse ?????????? */
...
#endif
typedef struct {
    oal_uint32 aul_rate[HMAC_STA_RSSI_LOG_NUM]; /* ????STA???????????? */
    oal_uint32 ul_index;   /* ????????????????????????????,???????????????????????????????? */
} hmac_sta_rx_rate_stru;

typedef struct {
    oal_uint32 auc_bssid[OAL_MAC_ADDR_LEN]; /* ??????????????20M??????????????AP */
    oal_int8 c_rssi;
    oal_uint8 auc_res[1];
...
typedef struct hmac_vap_tag {
    ...
    oal_uint32                      ul_last_roam_timestamp;                     /* ???????????????????????? */
    oal_bool_enum_uint8             en_roam_prohibit_on;                        /* ???????????? */
    oal_uint8                       en_last_roam_trigger;                       /* ??????????????????, ???????????????????????? */
    oal_uint8                       uc_diff_roam_trigger_cnt;                   /* ???????????? */
    ...
    oal_uint32                        ul_web_fail_roam_timestamp;             /* ?????????????????????????????????? */
    oal_uint32                        ul_assoc_timestamp;                     /* ?????????????????? */
    hmac_sta_rx_rate_stru             st_history_rx_rate;                     /* ????STA?????????????? */
    hmac_sta_20m_ap_stru              st_20m_ap_info;                         /* ????STA??????20M??????????????AP???? */
    ...
    oal_uint8     resv[2]; /* 2??resv???????? */
#endif
#ifdef _PRE_WLAN_FEATURE_FTM // FTM??????????gas init comeback????
