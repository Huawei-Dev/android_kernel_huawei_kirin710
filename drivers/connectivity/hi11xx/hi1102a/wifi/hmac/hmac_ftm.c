                                                 ...
                                                 mac_send_gas_init_req_stru *pst_gas_req)
{
    ...
    /* ??????????????BIT14?? BIT15??BIT31?? ??????ID??len???? */
    pst_gas_req->en_interworking_enable = (puc_ec_ie[5] & 0x80) >> 7;  /* en_interworking_enable??puc_ec_ie[5]??7bit */
    pst_gas_req->en_civic_enable = (puc_ec_ie[3] & 0x40) >> 6; /* en_civic_enable??puc_ec_ie[3]??6bit */
    pst_gas_req->en_geo_enable = (puc_ec_ie[3] & 0x80) >> 7; /* en_geo_enable??puc_ec_ie[3]??7bit */
...
oal_uint32 hmac_ftm_rx_gas_initial_response_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    us_index += 2;  /* 2????????category??public action???? */
