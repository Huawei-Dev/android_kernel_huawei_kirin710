                                   ...
                                   oal_uint16 us_trans_id)
{
    ...
    /* us_trans_id????8??????(us_trans_id??1??????puc_data[MAC_80211_FRAME_LEN + 2]) */
    puc_data[MAC_80211_FRAME_LEN + 2] = (us_trans_id & 0x00FF);
    /* us_trans_id????8??????(us_trans_id??2??????puc_data[MAC_80211_FRAME_LEN + 3]) */
                                  ...
                                  oal_uint16 us_err_code)
{
    ...
    puc_data[MAC_80211_FRAME_LEN + 1] = (us_err_code & 0xFF00) >> 8; /* ??us_err_code??8?? */
...
oal_uint16 hmac_encap_notify_chan_width(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, oal_uint8 *puc_da)
{
    ...
    /* puc_data??MAC_80211_FRAME_LEN + 2byte????en_bandwidth */
