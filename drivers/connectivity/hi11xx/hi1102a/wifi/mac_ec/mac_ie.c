                              ...
                              oal_uint16 *pus_amsdu_maxsize)
{
    ...
    *pus_ht_cap_info = oal_make_word16(puc_payload[us_offset], puc_payload[us_offset + 1]);
    ...
    /* ????AP 40M??DSSS/CCK????????????bit_dsss_cck_mode_40mhz????12bit?? */
    pst_ht_hdl->bit_dsss_cck_mode_40mhz = ((*pus_ht_cap_info & BIT12) >> 12);

    /* ????AP L-SIG TXOP ????????????????bit_lsig_txop_protection????15bit?? */
    ...
    oal_uint8 *puc_frame_body, oal_uint16 us_frame_len, oal_uint16 us_offset, oal_uint8 uc_curr_chan)
{
    ...
    if ((puc_ie_start_addr != OAL_PTR_NULL) && (puc_ie_start_addr[1] == MAC_DSPARMS_LEN)) {
        uc_chan_num = puc_ie_start_addr[2]; /* byte2?????????? */
...
oal_uint32 mac_ie_proc_ext_cap_ie(mac_user_stru *pst_mac_user, oal_uint8 *puc_payload)
{
    ...
    if ((uc_len >= MAC_IE_HDR_LEN) && (uc_len <= 8)) { /* 8????auc_cap?????????? */
                                            ...
                                            oal_uint8 *puc_output_len)
{
    ...
    *puc_output_len = 0;
    ...
    /* |1       |1      |1                        | */
    /******************************************************************/
    pauc_buffer[0] = 62; /* 62????????????????ie */
    pauc_buffer[1] = 1;

    switch (en_bw) {
        case WLAN_BAND_WIDTH_20M:
            pauc_buffer[2] = 0; /* byte2??????????????0???????????????? */
            break;

        case WLAN_BAND_WIDTH_40PLUS:
        case WLAN_BAND_WIDTH_80PLUSPLUS:
        case WLAN_BAND_WIDTH_80PLUSMINUS:
            pauc_buffer[2] = 1; /* byte2??????????????1???????????????????????? */
            break;

        case WLAN_BAND_WIDTH_40MINUS:
        case WLAN_BAND_WIDTH_80MINUSPLUS:
        case WLAN_BAND_WIDTH_80MINUSMINUS:
            pauc_buffer[2] = 3; /* byte2??????????????3???????????????????????? */
            break;
                                    ...
                                    oal_uint8 *puc_output_len)
{
    ...
    *puc_output_len = 0;
    ...
    /* -------------------------------------------------------------- */
    /* |1  |1      |1            |1                |1 */
    pauc_buffer[0] = 194; /* 194????????????????IE */
    pauc_buffer[1] = 3; /* 3????Length */
    switch (en_bw) {
        case WLAN_BAND_WIDTH_20M:
        case WLAN_BAND_WIDTH_40PLUS:
        case WLAN_BAND_WIDTH_40MINUS:
            pauc_buffer[2] = 0; /* byte2???????????????? */
            pauc_buffer[3] = 0; /* byte3????????????seg1 */
            break;

        case WLAN_BAND_WIDTH_80PLUSPLUS:
            pauc_buffer[2] = 1; /* byte2???????????????? */
            pauc_buffer[3] = uc_channel + 6; /* byte3????????????seg1??????????????????20??6?????? */
            break;

        case WLAN_BAND_WIDTH_80PLUSMINUS:
            pauc_buffer[2] = 1; /* byte2???????????????? */
            pauc_buffer[3] = uc_channel - 2; /* byte3????????????seg1??????????????????20??-2?????? */
            break;
    ...
    pauc_buffer[4] = 0; /* byte4????????????seg1,????????????80M + 80M */

    *puc_output_len = 5; /* 5??????ie?????? */
...
oal_uint32 mac_ie_proc_chwidth_field(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 uc_chwidth)
{
    ...
    wlan_bw_cap_enum_uint8 en_bwcap_user; /* user?????????????? */
...
oal_uint32 mac_ie_proc_obss_scan_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload)
{
    ...
    ***************************************************************************/
    ...
    /* ??6byte????7byte??????trigger_scan_interval??16bit?? */
    ...
    /* ??2byte????3byte??????ul_dot11OBSSScanPassiveDwell??16bit?? */
    mac_mib_set_OBSSScanPassiveDwell(pst_mac_vap, oal_make_word16(puc_payload[2], puc_payload[3]));
    /* ??4byte????5byte??????ul_dot11OBSSScanActiveDwell??16bit?? */
...
oal_uint32 mac_ie_proc_vht_opern_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload, mac_user_stru *pst_mac_user)
{
    ...
    /* MAC_IE_HDR_LEN????2??byte????Basic MCS Set */
    ...
    /* byte 3??byte 4??????????us_basic_mcs_set_all_user??16bit?? */
