...
oal_void mac_set_md_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    ***************************************************************************/
    if (pst_mac_vap->pst_mib_info->st_wlan_mib_fast_bss_trans_cfg.en_dot11FTOverDSActivated == OAL_TRUE) {
        *(puc_buffer + 4) += 1; /* FT??????(puc_buffer????4byte)??1 */
    }
    if (pst_mac_vap->pst_mib_info->st_wlan_mib_fast_bss_trans_cfg.en_dot11FTResourceRequestSupported == OAL_TRUE) {
        *(puc_buffer + 4) += 2; /* FT??????(puc_buffer????4byte)??2 */
    }
    *puc_ie_len = 5; /* Element ID??Length??MDID??FT Capbility & Policy??5???? */
}
oal_void mac_set_rde_ie(oal_void *pst_mac_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    *(puc_buffer + 1) = 4; /* Length????4 */
    *(puc_buffer + 2) = 0; /* RDE Identifier??(puc_buffer????2byte)???? */
    *(puc_buffer + 3) = 0; /* Res Dscr Cnt??(puc_buffer????3byte)???? */
    *(puc_buffer + 4) = 0; /* Status Code??(puc_buffer????4??5byte)???? */
    *(puc_buffer + 5) = 0;

    *puc_ie_len = 6; /* rde ie?? Element ID?? Length??RDE Identifier??Res Dscr Cnt??Status Code????6???? */
}
...
***********************************************************************************************/
    ...
    *(puc_buffer + 1) = 55; /* Length????55 */
    uc_len += 2; /* ????Element ID??Length(??2????) */
