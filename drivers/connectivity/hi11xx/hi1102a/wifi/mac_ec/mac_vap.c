...
oal_void mac_vap_init_mib_11n(mac_vap_stru *pst_mac_vap)
{
    ...
    pst_mib_info->st_phy_ht.ul_dot11NumberOfSpatialStreamsImplemented = 2; /* 2?????????????????????? */
    ...
    pst_mib_info->st_wlan_mib_ht_sta_cfg.ul_dot11MaxRxAMPDUFactor = 3; /* 3????????????AMPDU???? */
    pst_mib_info->st_wlan_mib_ht_sta_cfg.ul_dot11MinimumMPDUStartSpacing = 5; /* 5????????MPDU???????? */
    pst_mib_info->st_wlan_mib_ht_sta_cfg.en_dot11PCOOptionImplemented = OAL_FALSE;
    pst_mib_info->st_wlan_mib_ht_sta_cfg.ul_dot11TransitionTime = 3; /* 3???????????? */
...
oal_void mac_vap_init_mib_11ac(mac_vap_stru *pst_mac_vap)
{
    ...
    pst_mib_info->st_wlan_mib_vht_sta_config.ul_dot11VHTMaxRxAMPDUFactor = 7; /* 7???? VHT????????AMPDU???? */
...
oal_void mac_vap_init_legacy_rates(mac_vap_stru *pst_vap, mac_data_rate_stru *pst_rates)
{
    ...
    pst_vap->st_curr_sup_rates.uc_min_rate = 6; /* 6???????????????? */
    pst_vap->st_curr_sup_rates.uc_max_rate = 24; /* 24???????????????? */
    ...
    for (uc_rate_index = 0; uc_rate_index < uc_rates_num; uc_rate_index++) {
        ...
        /* Basic Rates */
        if ((puc_orig_rate->uc_mbps == 6) || (puc_orig_rate->uc_mbps == 12) || /* ??????????????6??12??24 */
            ...
            uc_curr_rate_index++;
        } else if ((puc_orig_rate->uc_mbps == 9) || (puc_orig_rate->uc_mbps == 18) || /* ??????????????9??18 */
                   (puc_orig_rate->uc_mbps == 36) || (puc_orig_rate->uc_mbps == 48) || /* ??????????????36??48??54 */
...
oal_void mac_vap_init_11b_rates(mac_vap_stru *pst_vap, mac_data_rate_stru *pst_rates)
{
    ...
    pst_vap->st_curr_sup_rates.uc_max_rate = 2; /* 2???????????????? */
    ...
    for (uc_rate_index = 0; uc_rate_index < uc_rates_num; uc_rate_index++) {
        ...
        /* Basic Rates */
        if ((puc_orig_rate->uc_mbps == 1) || (puc_orig_rate->uc_mbps == 2) || /* ??????????????1??2 */
            ((pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
             ((puc_orig_rate->uc_mbps == 5) || (puc_orig_rate->uc_mbps == 11)))) { /* ??????????????5??11 */
...
oal_void mac_vap_init_11g_mixed_one_rates(mac_vap_stru *pst_vap, mac_data_rate_stru *pst_rates)
{
    ...
    pst_vap->st_curr_sup_rates.uc_min_rate = 1;
    pst_vap->st_curr_sup_rates.uc_max_rate = 11; /* 11???????????????? */
    ...
    for (uc_rate_index = 0; uc_rate_index < uc_rates_num; uc_rate_index++) {
        ...
        if ((puc_orig_rate->uc_mbps == 1)
            || (puc_orig_rate->uc_mbps == 2) /* ??????????????2 */
            || (puc_orig_rate->uc_mbps == 5) /* ??????????????5 */
            || (puc_orig_rate->uc_mbps == 11)) { /* ??????????????11 */
            puc_curr_rate->uc_mac_rate |= 0x80;
        }
    }
}
...
oal_void mac_vap_init_11g_mixed_two_rates(mac_vap_stru *pst_vap, mac_data_rate_stru *pst_rates)
{
    ...
    for (uc_rate_index = 0; uc_rate_index < uc_rates_num; uc_rate_index++) {
        ...
        if ((puc_orig_rate->uc_mbps == 1)
            ...
            || (puc_orig_rate->uc_mbps == 5) /* ??????????????5 */
            || (puc_orig_rate->uc_mbps == 11) /* ??????????????11 */
            || (puc_orig_rate->uc_mbps == 6) /* ??????????????6 */
            || (puc_orig_rate->uc_mbps == 12) /* ??????????????12 */
            || (puc_orig_rate->uc_mbps == 24)) { /* ??????????????24 */
            puc_curr_rate->uc_mac_rate |= 0x80;
        }
    }
}
...
oal_void mac_vap_init_11n_rates(mac_vap_stru *pst_mac_vap, mac_device_stru *pst_mac_dev)
{
    ...
    if (pst_mac_dev->en_nss_num == WLAN_SINGLE_NSS) {
        ...
        if ((pst_mac_vap->st_channel.en_bandwidth == WLAN_BAND_WIDTH_40MINUS) ||
            ...
            /* 40M ????MCS32 */
            /* ???? RX MCS 32,????????(auc_dot11SupportedMCSRxValue[4])??1 */
            pst_mib_info->st_supported_mcsrx.auc_dot11SupportedMCSRxValue[4] = (oal_uint8)0x01;
            /* ???? RX MCS 32,????????(auc_dot11SupportedMCSTxValue[4] )??1 */
        ...
        pst_mib_info->st_phy_ht.ul_dot11TxMaximumNumberSpatialStreamsSupported = 2; /* 2????Tx?????????????????? */
        pst_mib_info->st_supported_mcsrx.auc_dot11SupportedMCSRxValue[0] = 0xFF; /* ???? RX MCS 0-7??8????????1 */
    ...
    wlan_prot_mode_enum_uint8 en_prot_mode, mac_cfg_rts_tx_param_stru *pst_rts_tx_param)
{
    ...
    if ((en_prot_mode == WLAN_PROT_ERP) && (en_flag == OAL_SWITCH_ON)) {
        ...
        pst_rts_tx_param->auc_protocol_mode[2] = WLAN_11B_PHY_PROTOCOL_MODE; /* auc_protocol_mode[2] ???????? 11b CCK */
        pst_rts_tx_param->auc_rate[2] = WLAN_SHORT_11b_11_M_BPS; /* RTS[3]????11Mbps */
        pst_rts_tx_param->auc_protocol_mode[3] = WLAN_11B_PHY_PROTOCOL_MODE; /* auc_protocol_mode[3] ???????? 11b CCK */
        pst_rts_tx_param->auc_rate[3] = WLAN_LONG_11b_1_M_BPS; /* RTS[3]????1Mbps */
    } else {
        ...
        /* auc_protocol_mode[2] ???????? 11g/a OFDM */
        pst_rts_tx_param->auc_protocol_mode[2] = WLAN_LEGACY_OFDM_PHY_PROTOCOL_MODE;
        pst_rts_tx_param->auc_rate[2] = WLAN_LEGACY_OFDM_24M_BPS; /* RTS[2]????24Mbps */

        if (pst_rts_tx_param->en_band == WLAN_BAND_2G) {
            /* auc_protocol_mode[3] 2G ???????? 11b CCK */
            pst_rts_tx_param->auc_protocol_mode[3] = WLAN_11B_PHY_PROTOCOL_MODE;
            pst_rts_tx_param->auc_rate[3] = WLAN_LONG_11b_1_M_BPS; /* 2G??RTS[3]????1Mbps */
        } else {
            /* auc_protocol_mode[3] 5G ???????? 11g/a OFDM */
            pst_rts_tx_param->auc_protocol_mode[3] = WLAN_LEGACY_OFDM_PHY_PROTOCOL_MODE;
            pst_rts_tx_param->auc_rate[3] = WLAN_LEGACY_OFDM_24M_BPS; /* 5G??RTS[3]????24Mbps */
        }
    }
}
...
oal_uint32 mac_mib_init_ft_cfg(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mde)
{
    ...
    if (pst_hmac_vap->en_auth_mode != WLAN_WITP_AUTH_FT) {
     ...
     /* ????puc_mde??????????????puc_mde??0????????MD IE????54????puc_mde??1??????3 */
    ...
    if (puc_mde[4] & 2) { /* ????2????puc_mde[4]????1bit */
 ...
 */
        ...
        case VOWIFI_SET_PERIOD: {
            ...
            /* ????ms,??????1s??30s??(* 1000), the period of monitor the RSSI when host suspended */
...
{
    mac_mib_set_OBSSScanPassiveDwell(pst_mac_vap, 20); /* ????MIB?? dot11OBSSScanPassiveDwell ??????20 */
    mac_mib_set_OBSSScanActiveDwell(pst_mac_vap, 10); /* ????MIB?? dot11OBSSScanActiveDwell ??????10 */
    mac_mib_set_BSSWidthTriggerScanInterval(pst_mac_vap, 300); /* ????MIB?? dot11BSSWidthTriggerScanInterval ????300 */
    mac_mib_set_OBSSScanPassiveTotalPerChannel(pst_mac_vap, 200); /* ????dot11OBSSScanPassiveTotalPerChannel ????200 */
    mac_mib_set_OBSSScanActiveTotalPerChannel(pst_mac_vap, 20); /* ????MIB?? dot11OBSSScanActiveTotalPerChannel ????20 */
    /* ????MIB?? dot11BSSWidthChannelTransitionDelayFactor ??????5 */
    mac_mib_set_BSSWidthChannelTransitionDelayFactor(pst_mac_vap, 5);
    mac_mib_set_OBSSScanActivityThreshold(pst_mac_vap, 25); /* ????MIB?? dot11OBSSScanActivityThreshold ??????25 */
}
