...
enum chr_wifi_error_enum {
    ...
    CHR_WIFI_DRV_ERROR_ONLY_ONE_INVALID_BSS = 0x01, /* ????????????????????BSS????BSS???? */
    CHR_WIFI_DRV_ERROR_INVALID_TARGET_BSS = 0x02, /* ??????????bss???? */
    CHR_WIFI_DRV_ERROR_MORE_THAN_ONE_BSS = 0x03, /* ????????????????bssid */
    CHR_WIFI_DRV_ERROR_ROAM_PING_PONG = 0x04, /* ???????????? */
