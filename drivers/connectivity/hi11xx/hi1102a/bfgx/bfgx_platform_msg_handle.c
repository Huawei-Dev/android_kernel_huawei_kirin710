...
STATIC platform_msg_handler g_sys_msg_handle[SYS_INF_BUTT] = {
    ...
    wcpu_message_handle,         // SYS_INF_WIFI_OPEN = 0x0F,              WCPU????????
    ...
    wcpu_message_handle,         // SYS_INF_WIFI_CLOSE = 0x14,             WCPU????????
    ...
    wcpu_message_handle,         // SYS_INF_WIFI_MEM_DUMP_COMPLETE = 0X18, bfgx????????MEM DUMP??????
    ...
    bfgx_uart_msg_handle,        // SYS_INF_BAUD_CHG_2M_REQ = 0x1c,        device??host????????2M??????
    bfgx_uart_msg_handle,        // SYS_INF_BAUD_CHG_6M_REQ = 0x1d,        device??host????????6M??????
    bfgx_uart_msg_handle,        // SYS_INF_BAUD_CHG_COMP_ACK = 0x1e,      device??host??????????????????????????ACK
