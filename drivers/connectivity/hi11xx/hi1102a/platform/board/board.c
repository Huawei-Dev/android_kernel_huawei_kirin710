...
int32 ssi_write32(uint32 addr, uint16 value)
{
    ...
    addr_half_word_high = (addr >> 16) & 0xffff; /* ??32??????????16?? */
...
int32 ssi_read_value32(uint32 addr, uint32 *value, int16 last_high_addr)
{
    ...
    *value = ((reg << 16) | *value); /* ??32??????????16????????????ssi????????16?????????? */
...
ssi_reg_info g_hi1103_pcie_dbi_full = { 0x40102000, 0x900,  SSI_RW_DWORD_MOD };         /* ???????????????? */
                                   ...
                                   int seg_index, int32_t seg_size)
{
    ...
    for (j = 0; j < seg_size; j += step) {
        ...
        last_high_addr = (ssi_address >> 16); /* ??????16?????? */
