...
typedef enum {
    ...
    OAL_ERR_CODE_WAKEUP_PROCESSING = (OAL_ERR_CODE_PM_BASE + 3),   /* ?????????????? */
    OAL_ERR_CODE_SEMA_TIMEOUT = (OAL_ERR_CODE_PM_BASE + 4),       /* ?????????????? */
    OAL_ERR_CODE_WAKEUP_FAIL_PROC = (OAL_ERR_CODE_PM_BASE + 5),   /* ?????????????? */
    OAL_ERR_CODE_WAKEUP_RETRY = (OAL_ERR_CODE_PM_BASE + 6),       /* ?????????????? */
    OAL_ERR_CODE_SLEEP_FAIL = (OAL_ERR_CODE_PM_BASE + 7),         /* ???????? */
    OAL_ERR_CODE_SLEEP_FORBID = (OAL_ERR_CODE_PM_BASE + 8),       /* ?????????? */
